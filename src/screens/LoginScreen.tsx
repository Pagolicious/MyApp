  ...
  const loginWithEmailAndPassword = async () => {
    try {
      ...
      if (currentUser) {
        ...
        const userCollectionRef = firestore().collection('users').doc(currentUser.uid).collection('names')
        const namesSnapshot = await userCollectionRef.get()
        ...
        if (!namesSnapshot.empty) {
          ...
          Alert.alert("Logged In")
          navigation.navigate("NamePage", { uid: currentUser.uid })
        }
      } else {
    ...
    <View style={styles.container}>
      <View style={styles.imageBackground}>
        ...
        <View style={styles.inputContainer}>
          <MyTextInput value={email} onChangeText={(text: string) => setEmail(text.trim())} placeholder="Email" />
          <MyTextInput value={password} onChangeText={(text: string) => setPassword(text.trim())} placeholder="Password" secureTextEntry />
